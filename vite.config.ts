...
interface RenderJob { id: string; directory: string; frames: string; settings: Record<string, any>; total: number; audio?: string; output?: string; process?: ChildProcess }
function exportBridge(): Plugin {
  const jobs = new Map<string, RenderJob>(), tempRoot = resolve(".temp");
  return { name: "algowzxd-export-bridge", configureServer(server) { server.middlewares.use(async (request: any, response: any, next: any) => {
    const parsed = new URL(request.url ?? "/", "http://local"), url = parsed.pathname;
    if (!url.startsWith("/api/export/")) return next();
    try {
      if (url === "/api/export/status") { const path = parsed.searchParams.get("path") || "ffmpeg"; const probe = spawnSync(path, ["-version"], { windowsHide: true }); return json(response, 200, { ready: probe.status === 0, path: probe.status === 0 ? path : "" }); }
      if (url === "/api/export/start") {
        const input = await readJson(request, 1_000_000), id = randomUUID(), directory = join(tempRoot, `render-${id}`), frames = join(directory, "frames"); await mkdir(frames, { recursive: true });
        jobs.set(id, { id, directory, frames, settings: input.settings as Record<string, any>, total: Number(input.total) || 0 }); return json(response, 200, { id });
      }
      const id = parsed.searchParams.get("id") || String((request.method === "POST" && request.headers["content-type"]?.includes("json") ? (await readJson(request, 100_000)).id : "") || ""), job = jobs.get(id);
      if (!job) return json(response, 404, { error: "Export job was not found." });
      if (url === "/api/export/frame") { const index = Number(parsed.searchParams.get("index")); if (!Number.isInteger(index) || index < 0 || index >= job.total) return json(response, 400, { error: "Invalid frame index." }); await writeFile(join(job.frames, `frame_${String(index).padStart(6, "0")}.png`), await readBuffer(request, 50_000_000)); return json(response, 200, { ok: true }); }
      if (url === "/api/export/audio") { const extension = extname(parsed.searchParams.get("name") || "") || ".audio"; job.audio = join(job.directory, `audio${extension}`); await writeFile(job.audio, await readBuffer(request, 200_000_000)); return json(response, 200, { ok: true }); }
      if (url === "/api/export/cancel") { job.process?.kill(); await rm(job.directory, { recursive: true, force: true }); jobs.delete(id); return json(response, 200, { canceled: true }); }
      if (url === "/api/export/finish") { try { await finishRender(job); } catch (error) { await rm(job.directory, { recursive: true, force: true }); jobs.delete(id); throw error; } return json(response, 200, { fileName: outputName(job), downloadUrl: `/api/export/file?id=${encodeURIComponent(id)}` }); }
      if (url === "/api/export/file") { if (!job.output) return json(response, 409, { error: "Export is not ready." }); response.statusCode = 200; response.setHeader("content-disposition", `attachment; filename="${outputName(job)}"`); response.setHeader("content-type", job.settings.format === "mp4" ? "video/mp4" : job.settings.format === "webm" ? "video/webm" : "application/zip"); createReadStream(job.output).pipe(response).on("finish", () => { if (!job.settings.keepFrames) setTimeout(() => { void rm(job.directory, { recursive: true, force: true }); jobs.delete(id); }, 30_000); }); return; }
      return json(response, 404, { error: "Unknown export operation." });
    } catch (reason) { return json(response, 500, { error: reason instanceof Error ? reason.message : "Export failed." }); }
  }); } };
}

async function finishRender(job: RenderJob) {
  if (job.settings.format === "png") { job.output = join(job.directory, "algowzxd-png-sequence.zip"); await run(job, "powershell.exe", ["-NoProfile", "-Command", "Compress-Archive -Path $args[0] -DestinationPath $args[1] -Force", join(job.frames, "*.png"), job.output]); return; }
  const extension = job.settings.format === "webm" ? ".webm" : ".mp4"; job.output = join(job.directory, `algowzxd-export${extension}`);
  const fps = String(job.settings.fps || 60), crf = String(({ Draft: 28, Standard: 23, High: 18, Maximum: 15 } as Record<string, number>)[job.settings.quality] ?? 18), ffmpeg = job.settings.ffmpegPath || "ffmpeg";
  const args = ["-y", "-framerate", fps, "-i", join(job.frames, "frame_%06d.png")]; if (job.audio) args.push("-i", job.audio);
  if (job.settings.format === "webm") args.push("-c:v", "libvpx-vp9", "-b:v", "0", "-crf", crf, "-pix_fmt", job.settings.background === "transparent" ? "yuva420p" : "yuv420p", ...(job.audio ? ["-c:a", "libopus"] : []));
  else args.push("-c:v", "libx264", "-preset", "medium", "-crf", crf, "-pix_fmt", "yuv420p", ...(job.audio ? ["-c:a", "aac", "-ar", "48000"] : []));
  args.push("-r", fps, ...(job.audio ? ["-shortest"] : []), job.output); await run(job, ffmpeg, args);
  if (!job.settings.keepFrames) await rm(job.frames, { recursive: true, force: true });
}
function run(job: RenderJob, command: string, args: string[]) { return new Promise<void>((resolveRun, reject) => { const child = spawn(command, args, { windowsHide: true }); job.process = child; let error = ""; child.stderr?.on("data", (chunk) => { error = (error + chunk).slice(-8000); }); child.on("error", reject); child.on("exit", (code) => code === 0 ? resolveRun() : reject(new Error(error || `${command} exited with code ${code}.`))); }); }
function outputName(job: RenderJob) { return job.settings.format === "png" ? "algowzxd-png-sequence.zip" : `algowzxd-${job.settings.width}x${job.settings.height}-${job.settings.fps}fps.${job.settings.format}`; }
...
function geminiBridge(apiKey: string, model: string, transcriptionModel: string): Plugin {
  ...
  return { name: "algowzxd-gemini-bridge", configureServer(server) {
    server.middlewares.use(async (request, response, next) => {
      const incoming = request as typeof request & { url?: string; method?: string };
      const url = incoming.url?.split("?")[0];
      if (url === "/api/ai/status") return json(response, 200, { configured: Boolean(sessionKey), model: sessionKey ? sessionModel : "local rules", performanceModel: sessionModel, transcriptionModel: sessionTranscriptionModel });
      if (url === "/api/ai/configure") {
        if (incoming.method !== "POST") return json(response, 405, { error: "POST required." });
        try {
          const input = await readJson(request, 4096);
          if (typeof input.apiKey !== "string" || input.apiKey.trim().length < 20) return json(response, 400, { error: "Enter a valid Gemini API key." });
          sessionKey = input.apiKey.trim();
          if (typeof input.model === "string" && input.model.trim()) sessionModel = input.model.trim();
        ...
        } catch (reason) { return json(response, 400, { error: reason instanceof Error ? reason.message : "Could not configure Gemini." }); }
      }
      if (url !== "/api/ai/transcribe" && url !== "/api/ai/analyze-performance") return next();
      if (incoming.method !== "POST") return json(response, 405, { error: "POST required." });
      if (!sessionKey) return json(response, 503, { error: "Gemini is not configured. Enter a key in AI Performance settings." });
      try {
      ...
      } catch (reason) { return json(response, 502, { error: reason instanceof Error ? reason.message : "Gemini request failed." }); }
    });
  } };
}

async function transcribe(apiKey: string, transcriptionModel: string, fallbackModel: string, input: Record<string, unknown>) {
  if (typeof input.data !== "string" || input.data.length > 30_000_000) throw new Error("Audio payload is missing or too large for direct transcription.");
  try { return await nativeTranscription(apiKey, transcriptionModel, input); }
  catch (reason) {
...
async function nativeTranscription(apiKey: string, model: string, input: Record<string, unknown>) {
  ...
  if (!start.ok) throw new Error(await googleError(start, "Could not start audio upload."));
  const uploadUrl = start.headers.get("x-goog-upload-url");
  if (!uploadUrl) throw new Error("Gemini did not return an audio upload URL.");
  ...
  if (!uploaded.ok) throw new Error(await googleError(uploaded, "Could not upload audio."));
  const file = (await uploaded.json() as { file?: { uri?: string; name?: string } }).file;
  if (!file?.uri) throw new Error("Gemini audio upload returned no file URI.");
  try {
    ...
    if (!response.ok) throw new Error(await googleError(response, `${model} transcription failed.`));
    return transcriptInputFromInteraction(await response.json(), Number(input.duration) || 0);
  } finally {
    if (file.name) void fetch(`https://generativelanguage.googleapis.com/v1beta/${file.name}`, { method: "DELETE", headers: { "x-goog-api-key": apiKey } });
  }
}

async function googleError(response: Response, fallback: string) { try { return (await response.json() as { error?: { message?: string } }).error?.message ?? fallback; } catch { return fallback; } }

function transcriptInputFromInteraction(raw: unknown, duration: number) {
  const interaction = raw as { output_text?: string; steps?: Array<{ content?: Array<{ annotations?: Array<Record<string, unknown>> }> }> };
  ...
  if (!annotations.length) throw new Error("Native transcription returned no word timestamps.");
...
async function analyze(apiKey: string, model: string, input: Record<string, unknown>) {
...
async function gemini(apiKey: string, model: string, parts: unknown[], responseJsonSchema?: unknown) {
...
const timedEvent = (types: string[]) => ({ type: "array", items: { type: "object", properties: { type: { type: "string", enum: types }, time: { type: "number" }, duration: { type: "number" }, strength: scalar }, required: ["type", "time", "duration", "strength"] } });
const objectSchema = (properties: Record<string, unknown>, required = Object.keys(properties)) => ({ type: "object", properties, required });
...
function readJson(request: any, limit: number): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => { let body = ""; request.setEncoding("utf8"); request.on("data", (chunk: string) => { body += chunk; if (body.length > limit) { reject(new Error("Request is too large.")); request.destroy(); } }); request.on("end", () => { try { resolve(JSON.parse(body || "{}")); } catch { reject(new Error("Invalid JSON request.")); } }); request.on("error", reject); });
}
function json(response: any, status: number, value: unknown) { response.statusCode = status; response.setHeader("content-type", "application/json; charset=utf-8"); response.end(JSON.stringify(value)); }
