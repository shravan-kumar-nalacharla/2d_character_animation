...
export function FaceAssetManager({ assets, face, onAssets, onFace }: Props) {
  ...
  return <div className="face-assets">
    ...
    <section className="inspector-section"><header>EXPRESSION MODE</header><div className="eye-mode-switch"><button className={assets.activeEyePack !== "modular-v2" ? "active" : ""} onClick={() => onAssets({ ...assets, activeEyePack: "raster-v1", modularExpressionV2: undefined })}>NORMAL</button><button className={assets.activeEyePack === "modular-v2" ? "active" : ""} onClick={() => onAssets({ ...assets, activeEyePack: "modular-v2", modularExpressionV2: undefined })}>ANIME</button></div>{assets.activeEyePack === "modular-v2" && <label className="select-field"><span>Anime expression</span><select value={assets.modularExpressionV2 ?? "auto"} onChange={(event) => { const value = event.target.value; onAssets({ ...assets, modularExpressionV2: value === "auto" ? undefined : value }); if (value !== "auto") onFace({ eyeExpression: modularExpressionEyePreset[value as keyof typeof modularExpressionEyePreset] }); }}><option value="auto">Auto / AI mapping</option>{modularExpressionSlugs.map((slug) => <option key={slug} value={slug}>{slug.replaceAll("-", " ")}</option>)}</select></label>}<a className="asset-qa-link" href="/assets/v2-qa">Open Anime asset QA contact sheet</a></section>
    ...
    <section className="inspector-section"><header>EYE ASSETS</header><select value={face.eyeExpression} onChange={(event) => { const eyeExpression = event.target.value as EyeExpression; if (assets.modularExpressionV2) onAssets({ ...assets, modularExpressionV2: undefined }); onFace({ eyeExpression, eyeSystem: { left: { ...face.eyeSystem.left, expression: "inherit" }, right: { ...face.eyeSystem.right, expression: "inherit" } } }); }}>{eyeExpressions.map((item) => <option key={item}>{item}</option>)}</select><div className="eye-upload-row">{(["left", "right"] as const).map((side) => <div key={side}><b>{side.toUpperCase()}</b><label>Replace<input type="file" accept="image/svg+xml,image/png,image/webp" onChange={(event) => void replaceEye(face.eyeExpression, side, event.target.files?.[0]).catch((reason) => setError(String(reason)))}/></label>{assets.eyeOverrides[face.eyeExpression]?.[side] && <button onClick={() => { const entry = { ...assets.eyeOverrides[face.eyeExpression] }; delete entry[side]; onAssets({ ...assets, eyeOverrides: { ...assets.eyeOverrides, [face.eyeExpression]: entry } }); }}>Reset</button>}</div>)}</div></section>
