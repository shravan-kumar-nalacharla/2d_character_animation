...
export function FaceRig({ face, calibration, assets, time, playing, headMotion = { rotation: 0, x: 0, y: 0 }, resolveAssetUrl = identity }: Props) {
  ...
  const shadowRage = face.faceFx === "angryShadow" || (face.faceFx === "auto" && autoShadowRage);
  ...
  const mouthHref = shadowRage && preview.mouth === "REST" ? "/production_character/illustrator2024/mouths/frown.svg" : assetState.mouthOverrides[preview.mouth]?.dataUrl ?? `${mouthPackBase}/${preview.mouth}.svg`, glasses = face.accessories?.sunglasses;
  return <g className="face-rig" pointerEvents="none">
    <defs><linearGradient id="eyeSparkle" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#2d3540"/><stop offset="1" stopColor="#080a0d"/></linearGradient></defs>
    ...
    <g transform={`rotate(${(face.mouthRotation ?? 0) + (mouthTransform.rotation ?? 0)} ${mouthX} ${mouthY})`}><image data-face-part="mouth" href={resolveAssetUrl(mouthHref)} x={mouthX - width / 2} y={mouthY - height / 2} width={width} height={height} /></g>
